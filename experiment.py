...
class LibraryManagementSystem:
    def __init__(self,root):
        ...
        def SelectBook(event=""):
            ...
            values=str(listBox.get(listBox.curselection()))
            x=values
            print(x)
            if (x=="Hindi"):
                self.bookid_var.set("BKID1456")
                self.booktitle_var.set("Litreture")
                self.auther_var.set("KC.SINHA")
    ...
    def update(self):
        conn=mysql.connector.connect(host="localhost",username="root",password="",database="lms")
        my_cursor=conn.cursor()
        my_cursor.execute("update library set Member=%s,ID=%s,FirstName=%s,LastName=%s,Address1=%s,Address2=%s,PostId=%s,Mobile=%s,Bookid=%s,Bookborrowed=%s,Auther=%s,datedue=%s,daysofbook=%s,latereturnfine=%s,dateovedue=%s,finalprice=%s where PNR_NO=%s",(
